...
public final class GeneratePdfTable {
	...
	public static BaseTable prepareCustomerTable(
			final CustomerInfo customerInfo, final PDDocument doc,
			final PDPage page) {
		if (null != customerInfo) {
			...
			try {
				custTable = new BaseTable(yStart, yStartNewPage, bottomMargin,
						tableWidth, margin, doc, page, true, drawContent);
				// Create Header row
				...
				cell = row.createCell(
						9,
						(null != customerInfo.getCustomerId()) ? String
								.valueOf(customerInfo.getCustomerId()) : "");
				cell.setFont(PDType1Font.HELVETICA_OBLIQUE);
				cell.setFontSize(6);
	...
	public static BaseTable prepareAccountTable(final AccountInfo acctInfo,
			final PDDocument doc, final PDPage page) {
		if (null != acctInfo) {
			...
			float bottomMargin = 70;
			try {
				acctTable = new BaseTable(yStart, yStartNewPage, bottomMargin,
						tableWidth, margin, doc, page, true, drawContent);
				// Create Header row
				...
				cell = row.createCell(25,
						String.valueOf(acctInfo.getAccountId()));
				cell.setFont(PDType1Font.HELVETICA);
				cell.setFontSize(6);
